...
public void shoot(final Proxy proxy,List<Shot> csc)
{
	...
	Collections.sort(shots, new Comparator<Shot>(){
		...
		public int compare(Shot arg0, Shot arg1) {
			if(arg0.getT_shot() < arg1.getT_shot()) {
				return -1;
			} else if(arg0.getT_shot() > arg1.getT_shot()){
				return 1;
			} else {
				return 0;
			}
	...
	if(!shots.isEmpty())
	{
			   long start_time = shots.getFirst().getT_shot();
			   ...
			   if(shots.size() < 3)
			   {
				   ...
				   if(shots.size() == 2)
				   {
					   ...
					   long wait_time = (_shot.getT_shot() - start_time)==0? start_time:(_shot.getT_shot()- start_time);
